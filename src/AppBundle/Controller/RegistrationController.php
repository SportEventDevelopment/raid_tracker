    ...
    public function register(Request $request)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
           ...
           if( (strlen($mdp) <8) || (!(preg_match("~[0-9]~", $mdp))) || (!(preg_match("~[A-Z]~", $mdp)))   ) {
             //echo ('votre mdp pas correct');die();
             $this->addFlash('notice','Au moins 8 charactères dont 1 chiffre et 1 majuscule !');
           }
           else {
             $response = $this->get('app.restclient')->post('api/users', $data);
               if($response) {
                  $this->addFlash('info','votre compte a été bien crée !');
